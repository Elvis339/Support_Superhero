...
let __client__ = ( async () => await constructClient() )()

const activecollab_projects = async (req, res, next) => {
    try {
        req.ActiveCollabProjects = await (await __client__)._get(`projects/${process.env.ACTIVECOLLAB_PROJECT_ID}`)
        next()
    } catch (error) {
        res.status(500).send({
...
const activecollab_tasks = async (req, res, next) => {
    try {
        req.ActiveCollabTasks = await (await __client__)._get(`projects/${process.env.ACTIVECOLLAB_PROJECT_ID}/tasks`)
        next()
    } catch (error) {
        res.status(500).send({
